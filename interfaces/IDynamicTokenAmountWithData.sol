...
interface IDynamicTokenAmountWithData {
    ...
    function getAmountForToken(uint256 paymentRequestId, address token, address payer, address data, uint256 dataId) external returns (uint256);
    ...
    function isTokenAccepted(uint256 paymentRequestId, address token, address payer, address data, uint256 dataId) external returns (bool);
 }
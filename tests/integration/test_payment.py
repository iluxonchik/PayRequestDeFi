...
from tests.configuration import PaymentRequestConfiguration, PaymentPrecondition, TokenAmount, PostPaymentAction
...
@given(configuration=payment_request_configuration_strategy(
    precondition_id=strategies.just(PaymentPrecondition.NONE),
    post_payment_action_id=strategies.just(strategies.integers(
        min_value=0,
        max_value=2,
    ))
), )
